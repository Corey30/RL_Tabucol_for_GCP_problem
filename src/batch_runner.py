...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="time_limit",
        default=18000,
        help="Time limit for run time of the algorithm, in seconds (default: 18000 = 5 hours)",
    )
    parser.add_argument(
    ...
    def run_job(i):
        ...
        aco_args = ""
        if args.use_aco:
            aco_args = f" --use-aco --aco-ants {args.aco_ants} --aco-iter {args.aco_iter}"
        cmd = f"python runner.py {args.policy} {args.graph} {args.k} -I {args.max_steps} -T {args.max_tabucol_iters} -B {args.beta} --time-limit {args.time_limit} --RL {args.RL}{aco_args}"
        print("running: ", cmd)
