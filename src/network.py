...
class GCPPPOPolicy(PPOPolicy):
    ...
    def update(self, sample_size: int, buffer, **kwargs) -> dict:
        result = super().update(sample_size, buffer, **kwargs)
        
        if 'loss' in result and result['loss']:
            self.last_policy_loss = float(np.mean(result['loss']))
        
        if 'loss/clip' in result and result['loss/clip']:
            self.last_policy_loss = float(np.mean(result['loss/clip']))
        
        if 'loss/vf' in result and result['loss/vf']:
            self.last_value_loss = float(np.mean(result['loss/vf']))
        
        if 'loss/ent' in result and result['loss/ent']:
            self.last_entropy = float(np.mean(result['loss/ent']))
